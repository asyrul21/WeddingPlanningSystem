...
router.post('/', middleware.isLoggedIn, function(req, res){
    ...
    var newGuest = {
        name: req.sanitize(req.body.guestname),
...
router.put('/:id', middleware.isLoggedIn, function(req, res){
    ...
    // res.redirect('/guestlist');
    const updatedGuest = {
        name: req.sanitize(updatedGuestTemp.name),
...
router.post('/search', function(req, res){
    ...
    const searchItem = req.sanitize(req.body.search);
    ...
    Guest.find({name : searchItem}, function(err, foundGuests){
        if(err){
            ...
            res.render('guestlist/show', { list: foundGuests, tempList:tempGuestlistExcel, total: 21});
        }
    });
